...
#### 3. **联系**
- **目的**：
  - 两者都是用于量化单细胞 RNA 测序中的基因表达水平，但 UMI Counts 提供了更准确和可靠的测量。

- **应用场景**：
  - cDNA Counts 和 UMI Counts 都可以用于单细胞 RNA 测序分析，但在处理技术噪声和零值方面，UMI Counts 通常表现更好。

#### 4. **总结**
- **UMI Counts** 更适合用于需要高精度和低技术噪声的单细胞 RNA 测序分析，尤其是在处理低表达基因和减少零值（dropout events）方面。
- **cDNA Counts** 在某些情况下仍然被使用，但需要更复杂的统计模型来处理数据。
